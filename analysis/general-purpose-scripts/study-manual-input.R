path_zip <- "./data-unshared/raw/Sample-APR.zip"
...
question_name <- path_files[1] %>% stringr::str_replace(".csv", "")
ls_old <- list()

for(question_i in seq_along(path_files) ){
  # i <- 1
  question_name <- path_files[question_i] %>% basename() %>% stringr::str_replace(".csv", "")
  ls_old[[question_name]] <- read.csv(path_files[question_i], header = TRUE, stringsAsFactors = TRUE)
}

ls_old[[question_name]] <- read.csv(
  paste0(folder_path,paste0(question_name,".csv")),
  header=TRUE,
...
adults <- ls_old[["q7a"]][1,2]
children <- ls_old[["q7a"]][2,2]
childHoH <- ls_old[["q5a"]][15,1]
paste("Clients Entering from Homeless Situations:",sprintf("%1.0f%%",100*ls_old[["q15"]][7,2]/(adults+childHoH)))
